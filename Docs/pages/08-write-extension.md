...
- `Has…` on `IThat<TException>` describes the exception as a standalone sentence ("has Message equal to …").
  It applies to an exception subject, including the thrown exception after `.Which`.
- `With…` on `ThatDelegateThrows<TException>` continues the sentence started by `Throws`
...
ungrammatical message, which is what the analyzer rule `aweXpect0003` reports for the built-in `Has…` expectations.
