...
public sealed class RequiresMemberMetadataTests
{
	...
	/// <remarks>
	///     Every public method that takes an equivalency options callback lets a value reach the comparison.
	/// </remarks>
	private static IEnumerable<MethodInfo> EquivalencyEntryPoints()
		=> typeof(EquivalencyExtensions).Assembly.GetTypes()
			...
			.Where(method => method.GetParameters().Any(TakesEquivalencyOptions));
