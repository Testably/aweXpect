	...
	public sealed partial class DidNotSignal
	{
		public sealed class AtLeastTests
		{
			...
			public async Task WhenNotTriggeredOftenEnough_ShouldSucceed()
			{
				...
				async Task Act() =>
					await That(signaler).DidNotSignal().AtLeast(2.Times()).WithCancellation(token);
			...
			public async Task WhenNotTriggeredWithParameter_ShouldSucceeded()
			{
				...
				async Task Act() =>
					await That(signaler).DidNotSignal().AtLeast(3.Times()).WithCancellation(token);
			...
			public async Task WhenTriggeredMoreOften_ShouldFail()
			{
				...
				async Task Act() =>
					await That(signaler).DidNotSignal().AtLeast(3.Times());
			...
			public async Task WhenTriggeredOftenEnough_ShouldFail()
			{
				...
				async Task Act() =>
					await That(signaler).DidNotSignal().AtLeast(2.Times());
