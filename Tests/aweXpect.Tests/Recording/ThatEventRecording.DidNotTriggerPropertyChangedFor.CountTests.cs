	...
	public sealed partial class DidNotTriggerPropertyChangedFor
	{
		public sealed class CountTests
		{
			...
			public async Task ShouldSupportAtLeast(int count, bool expectSuccess)
			{
				...
				NotifyMyValueAndOtherPropertyChanged(sut, count);
				...
				await That(Act).Throws<XunitException>().OnlyIf(!expectSuccess)
					.WithMessage("""
					             ...
					             has recorded the PropertyChanged event on sut for property MyValue less than twice,
			...
			public async Task Within_WhenCountIsReachedWithinTimeout_ShouldFail()
			{
				...
				_ = Task.Delay(20.Milliseconds())
					.ContinueWith(_ => NotifyMyValueAndOtherPropertyChanged(sut, 2));
				...
				await That(Act).Throws<XunitException>()
					.WithMessage("""
					             Expected that recording
					             has recorded the PropertyChanged event on sut for property MyValue less than twice within 0:05,
